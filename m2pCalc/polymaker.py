...
class PolyMaker:
    def __init__(self, head_tail: bool):
        self.smiles_req = {
            "ols": "[C,c;!$(C=O)][OH]",
        ...
        if head_tail == True:
            self.reactions = {
                ...
                "Urethane":{
                    ...
                    "[#6:0][#7;!$([#7+]):1]-[#6:2](=[#8:3])[Kr][O:5][C;!$(C=O):4]",
    ...
    @staticmethod
    def get_monomers(smi: str, stereochemistry: bool = True, headtail=True) -> List[str]:
        ...
        if ".." not in smi:
            if type(smi) == str:
                ...
                monomers = tuple([PolyMaker(head_tail=headtail).checksmile(smi_i) for smi_i in smi])
            elif type(smi) == tuple:
                monomers = smi
            else:
                monomers = None
            ...
            # M1R.M1S..M2R.M2S
            monomers = [
                [PolyMaker(head_tail=headtail).checksmile(enant) for enant in enants.split(".")]
    ...
    def thermoplastic(
        ...
        head_tail: boolean, by default False,
            For vinyl mechanism add head and tail for polymers.
        ...
        returnpoly = returnpoly.sort_index().sort_values("replicate_structure")
        returnpoly = returnpoly.explode("smiles_polymer", ignore_index=True)
        return returnpoly
    ...
    def __polymerizemechanism_thermoplastic(
        self, reactants, DP, mechanism, distribution=[], infinite_chain=False, head_tail=False, all_possibilities=False,
        rep=None
    ):
        ...
        if (mechanism == "vinyl") | (mechanism == "acrylate"):
            polydata = self.__poly_vinyl(reactants, DP, distribution, infinite_chain, head_tail=head_tail, all_possibilities=all_possibilities)
            ...
            polylist = [
                self.__poly_vinyl(reactants, DP, distribution, infinite_chain, head_tail=head_tail, all_possibilities=all_possibilities),
    ...
    def __poly_vinyl_init(self, mola, molb, head_tail=False):
        """performs propagation rxn of vinyl polymer"""

        # rxn definition
        if head_tail:
            rxn = AllChem.ReactionFromSmarts(
                "[C:1]=[C:2].[C:3]=[C:4]>>[Kr][C:1][C:2][C:3][C:4][Kr]"
            )
        else:
            rxn = AllChem.ReactionFromSmarts(
    ...
    def __poly_vinyl_init_c(self, mola, molb, molc, head_tail=False):
        """performs propagation rxn of vinyl polymer"""
        
        # rxn definition
        if head_tail:
            rxn = AllChem.ReactionFromSmarts(
                "[C:1]=[C:2].[C:3]=[C:4].[C:5]=[C:6]>>[Kr][C:1][C:2][C:3][C:4][C:5][C:6][Kr]"
            )
        else:
            rxn = AllChem.ReactionFromSmarts(
                "[C:1]=[C:2].[C:3]=[C:4].[C:5]=[C:6]>>[Kr][C:1][C:2][C:3][C:4][C:5][C:6][Xe]"
            )         
        # product creation and validation
        prod = rxn.RunReactants((mola, molb, molc))
        prodlist = [Chem.MolToSmiles(x[0]) for x in prod]

        molprodlist = [Chem.MolFromSmiles(p) for p in self._returnvalid(prodlist)]
        return molprodlist

    def __poly_vinyl_prop(self, mola, molb, head_tail=False):
        """performs propagation rxn of vinyl polymer"""

        # rxn definition
        if head_tail == True:
            rxn = AllChem.ReactionFromSmarts(
                "[Kr][C:0][C:1][Rn][C:2][C:3][Kr].[C:4]=[C:5]>>[Kr][C:0][C:1][C:2][C:3][C:4][C:5][Xe]"
            )
        else:
            rxn = AllChem.ReactionFromSmarts(
    ...
    def __poly_vinyl_prop_c(self, mola, molb, molc, head_tail=False):
        """performs propagation rxn of vinyl polymer"""

        # rxn definition
        if head_tail:
            rxn = AllChem.ReactionFromSmarts(
                "[C:0][C:1][Rn][C:2][C:3][Kr].[C:4]=[C:5].[C:6]=[C:7]>>[C:0][C:1][Rn][C:2][C:3][C:4][C:5][C:6][C:7][Kr]"
            )
        else:
            rxn = AllChem.ReactionFromSmarts(
                "[C:0][C:1][C:2][C:3][Xe].[C:4]=[C:5].[C:6]=[C:7]>>[C:0][C:1][C:2][C:3][C:4][C:5][C:6][C:7][Xe]"
            )            
    ...
    def __poly_vinyl_term(self, mola, molb, infinite_chain=False, single_rxn=False, head_tail=False):
        ...
        if head_tail:
            if single_rxn:
                rxn1 = AllChem.ReactionFromSmarts(
                    "[C:0]=[C:1].[C:2]=[C:3]>>[Kr][C:0][C:1][C:2][C:3][Xe]"
                )
                ...
                rxn1 = AllChem.ReactionFromSmarts(
                    "[C:0][C:1][C:2][C:3][Kr].[C:4]=[C:5]>>[C:0][C:1][Rn][C:2][C:3][C:4][C:5][Xe]"
                )
                prod = rxn1.RunReactants((mola, molb))
                # ring closes
                rxn2 = AllChem.ReactionFromSmarts(
                    "([Kr][C:0][C:1].[C:2][C:3][Xe])>>[Kr][C:1][C:0][C:3][C:2][Xe]"
                )
                ...
                # removes Kr
                rxn2 = AllChem.ReactionFromSmarts(
                    "[C:0][C:1][C:2][C:3][Kr]>>[Kr][C:1][C:0][Rn][C:3][C:2][Xe]"
                )
                prod = [rxn2.RunReactants((r,)) for r in list(itertools.chain(*prod))]
                prod = list(itertools.chain(*prod))
        else:
    ...
    def __poly_vinyl_term_c(self, mola, molb, molc, infinite_chain=False, single_rxn=False, head_tail=False):
        """performs termination rxn of vinyl polymer"""
        # rxn definition
        if head_tail:
            if single_rxn:
                rxn1 = AllChem.ReactionFromSmarts(
                    "[C:0]=[C:1].[C:2]=[C:3].[C:4]=[C:5]>>[Kr][C:0][C:1][C:2][C:3][C:4][C:5][Xe]"
                )
                prod = rxn1.RunReactants((mola, molb, molc))
                
            elif infinite_chain:
                # terminates and removes Xe
                rxn1 = AllChem.ReactionFromSmarts(
                    "[C:0][C:1][Rn][C:2][C:3][Kr].[C:4]=[C:5].[C:6]=[C:7]>>[C:0][C:1][Rn][C:2][C:3][C:4][C:5][C:6][C:7][Xe]"
                )
                prod = rxn1.RunReactants((mola, molb, molc))
                # ring closes
                rxn2 = AllChem.ReactionFromSmarts(
                    "([Kr][C:0][C:1].[C:2][C:3].[C:4][C:5][Kr])>>[Kr][C:1][C:0][C:3][C:2][C:5][C:4][Xe]"
                )
                prod = [rxn2.RunReactants((r,)) for r in list(itertools.chain(*prod))]
                prod = list(itertools.chain(*prod))
            else:
            ...
            if single_rxn:
                ...
                # ring closes
                rxn2 = AllChem.ReactionFromSmarts(
                    "([Kr][C:0][C:1].[C:2][C:3].[C:4][C:5][Xe])>>[C:1][C:0][C:3][C:2][C:4][C:5]"
                )
                prod = [rxn2.RunReactants((r,)) for r in list(itertools.chain(*prod))]
                prod = list(itertools.chain(*prod))
            else:
                # terminates and removes Xe
                rxn1 = AllChem.ReactionFromSmarts(
                    "[C:0][C:1][C:2][C:3][Xe].[C:4]=[C:5].[C:6]=[C:7]>>[C:0][C:1][C:2][C:3][C:4][C:5][C:6][C:7]"
                )
                prod = rxn1.RunReactants((mola, molb, molc))
                # removes Kr
                rxn2 = AllChem.ReactionFromSmarts(
                    "[C:0][C:1][C:2][C:3][C:4][C:5][Kr]>>[C:0][C:1][C:2][C:3][C:5][C:4]"
                )
                prod = [rxn2.RunReactants((r,)) for r in list(itertools.chain(*prod))]
                prod = list(itertools.chain(*prod))
        # preps for return
    ...
    def __poly_vinyl(
        self, reactants, DP=3, distribution=[], infinite_chain=False, crosslink=False, head_tail=False, all_possibilities=False
    ):
        ...
        try:
            ...
            if crosslink:
                ...
                dfmolC = pd.DataFrame(dfreactants.iloc[2]).T
            ...
            if len(reactants) == 1:
                if DP > 2:
                    ...
                    polymer = self.__poly_vinyl_init(molA, molA, head_tail=head_tail)
                    ...
                    for r in range(0, DP - 3):
                        ...
                        polymer = self.__poly_vinyl_prop(polymer, molA, head_tail=head_tail)
                    ...
                    polymer = self.__poly_vinyl_term(polymer, molA, infinite_chain, head_tail=head_tail)
                ...
                if DP == 2:
                    ...
                    polymer = self.__poly_vinyl_term(molA, molA, single_rxn=True, head_tail=head_tail)
            ...
            if len(reactants) == 2:
                if DP > 2:
                    ...
                    polymer = self.__poly_vinyl_init(molA, mol, head_tail=head_tail)
                    ...
                    for r in range(0, DP - 3):
                        ...
                        polymer = self.__poly_vinyl_prop(polymer, mol, head_tail=head_tail)
                    ...
                    polymer = self.__poly_vinyl_term(polymer, molB, infinite_chain, head_tail=head_tail)
                ...
                if DP == 2:
                    ...
                    polymer = self.__poly_vinyl_term(molA, molB, single_rxn=True, head_tail=head_tail)
            ...
            if len(reactants) == 3:
                if DP > 3:
                    ...
                    polymer = self.__poly_vinyl_init_c(molA, mol1, mol2, head_tail=head_tail)
                    ...
                    for r in range(0, DP - 3):
                        ...
                        polymer = self.__poly_vinyl_prop_c(polymer, molB, mol3, head_tail=head_tail)
                    ...
                    molC = (
                        dfmolC.sample(1, weights=dfmolC.distribution, replace=True)
                        .iloc[0]
                        .loc["mols"]
                    )
                    polymer = self.__poly_vinyl_term_c(polymer, mol4, molC, infinite_chain, head_tail=head_tail)
                ...
                if DP == 3:
                    ...
                    polymer = self.__poly_vinyl_term_c(molA, molB, molC, single_rxn=True, head_tail=head_tail)

            if all_possibilities == False:
                polymer = Chem.MolToSmiles(random.choice(polymer))
            elif all_possibilities == True:
                polymer_list = []

                count = 0

                while count <= 3:
                    polymer_smiles = Chem.MolToSmiles(polymer[count])
                    polymer_list.append(polymer_smiles)
